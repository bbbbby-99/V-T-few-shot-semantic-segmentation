...
for epoch in range(0,num_epoch):
    ...
    for i_iter, batch in enumerate(tqdm_gen):
        ...
        query_rgb,query_th, query_mask,support_rgb, support_th,support_mask,history_mask,sample_class,index= batch

        query_rgb = (query_rgb).cuda(0)
        query_th = (query_th).cuda(0)
        support_rgb = (support_rgb).cuda(0)
        support_th = (support_th).cuda(0)
        ...
        pred =model(query_rgb,query_th, support_rgb,support_th, support_mask,history_mask,mode='train')
    ...
    with torch.no_grad():
        ...
        valset.history_mask_list=[None] * 500
        best_iou = 0
        best_FBiou = 0
        for eva_iter in range(options.iter_time):
            all_inter, all_union, all_predict = [0] * 4, [0] * 4, [0] * 4
            inter = 0
            union = 0
            inter_bg = 0
            union_bg = 0
            for i_iter, batch in enumerate(valloader):
                ...
                pred= model(query_rgb,query_th, support_rgb, support_th,support_mask,history_mask,mode='val')
                pred_softmax = F.softmax(pred, dim=1).data.cpu()
                # update history mask
                for j in range(support_mask.shape[0]):
                    sub_index = index[j]
                ...
                inter_list_bg, union_list_bg, _, num_predict_list_bg = get_iou_v1(query_mask, pred_label,mode='background')
                for j in range(query_mask.shape[0]):#batch size
                    all_inter[sample_class[j] - (options.fold * 4 + 1)] += inter_list[j]
                    all_union[sample_class[j] - (options.fold * 4 + 1)] += union_list[j]
                    inter += inter_list[j]
                    union += union_list[j]
                    inter_bg += inter_list_bg[j]
                    union_bg += union_list_bg[j]

            FBiou = ((inter/ union)+(inter_bg / union_bg))/2
        ...
        if best_FBiou>highest_FBiou:
            highest_FBiou = best_FBiou
        ...
        print('IOU for this epoch: %.4f' % (best_FBiou))
    ...
    print('best epoch:%d ,iout:%.4f,fbiout:%.4f' % (best_epoch, highest_iou,highest_FBiou))
